...
from .adaptor import PriorAdaptor
...
class SGPANet(nn.Module):
    def __init__(self, n_cat=6, nv_prior=1024, num_structure_points=128):
        ...
        conv1d_stpts_prob_modules.append(
            nn.Conv1d(
                in_channels=256, out_channels=self.num_structure_points, kernel_size=1
            )
        )
    ...
    def forward(self, points, img, choose, cat_id, prior):
        ...
        weighted_xyz = torch.sum(
            self.lowrank_projection[:, :, :, None] * input_points[:, None, :, :], dim=2
        )

        weighted_points_features = torch.sum(
            self.lowrank_projection[:, None, :, :] * points[:, :, None, :], dim=3
        )
        weighted_img_features = torch.sum(
            self.lowrank_projection[:, None, :, :] * emb[:, :, None, :], dim=3
        )

        # category-specific features
        cat_points = self.category_local(prior)  # bs x 64 x n_pts
        cat_color = self.prior_enricher(
            cat_points, weighted_points_features, weighted_img_features
        )
        ...
        assign_feat = torch.cat(
            (
                inst_local,
                inst_global.repeat(1, 1, n_pts),
                cat_global.repeat(1, 1, n_pts),
            ),
            dim=1,
        )  # bs x 2176 x n_pts
        ...
        deform_feat = torch.cat(
            (cat_local, cat_global.repeat(1, 1, nv), inst_global.repeat(1, 1, nv)),
            dim=1,
        )  # bs x 2112 x n_pts

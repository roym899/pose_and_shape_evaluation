...
class DeepPriorDeformer(nn.Module):
    ...
    def forward(self, rgb_local, pts_local, prior_local, index):
        ...
        ## attention
        atten_feat = torch.cat(
            (pts_local, rgb_local, prior_global.repeat(1, 1, npoint)), dim=1
        )
        ...
        Qo = torch.bmm(
            Qv.transpose(1, 2).detach(),
            F.softmax(attention.detach(), dim=2).permute(0, 2, 1).contiguous(),
        )
        new_prior_local = torch.bmm(
            prior_local, F.softmax(attention, dim=2).permute(0, 2, 1).contiguous()
        )
...
class PointNet2MSG(nn.Module):
    def __init__(self, radii_list, use_xyz=True):
        ...
        self.FP_modules.append(
            PointnetFPModule(mlp=[c_out_3 + c_out_2, 512, 512], bn=True)
        )
...
class PoseNet(nn.Module):
    ...
    def forward(self, rgb_local, pts_local, prior_local, pts, index):
        A, new_prior_local, Qv, Qo = self.deformer(
            rgb_local, pts_local, prior_local, index
        )

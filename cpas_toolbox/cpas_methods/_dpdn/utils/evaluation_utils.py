...
def compute_combination_RT_degree_cm_symmetry(
    RT_1, RT_2, scale, class_id, handle_visibility, synset_names
):
    """
...
def compute_combination_3d_matches(
    ...
    pred_matches = -1 * np.ones(
        [num_degree_thes, num_shift_thes, num_iou_3d_thres, num_pred]
    )
    gt_matches = -1 * np.ones(
        [num_degree_thes, num_shift_thes, num_iou_3d_thres, num_gt]
    )
            ...
            for t, shift_thres in enumerate(shift_thesholds):
                for i in range(len(pred_boxes)):
                    ...
                    low_score_idx = np.where(
                        overlaps[i, sorted_ixs_by_iou] < score_threshold
                    )[0]
                    ...
                    for j in sorted_ixs_by_iou:
                        ...
                        if (
                            iou < iou_thres
                            or r_error > degree_thres
                            or t_error > shift_thres
                        ):
                            break
...
def compute_combination_mAP(
    ...
    pred_matches_all = [
        np.zeros((num_degree_thres, num_shift_thres, num_iou_thres, 0))
    ...
    for progress, result in tqdm(enumerate(final_results)):
        ...
        for cls_id in range(1, num_classes):
            ...
            assert (
                pred_matches_all[cls_id].shape[-1] == pred_scores_all[cls_id].shape[-1]
            )
    ...
    print(
        "IoU75, 5  degree,  5% translation: {:.2f}".format(
            aps[
                -1,
                degree_thres_list.index(5),
                shift_thres_list.index(0.05),
                iou_thres_list.index(0.75),
            ]
            * 100
        )
    )
    print(
        "IoU75, 10 degree,  5% translation: {:.2f}".format(
            aps[
                -1,
                degree_thres_list.index(10),
                shift_thres_list.index(0.05),
                iou_thres_list.index(0.75),
            ]
            * 100
    ...
    print(
        "IoU75, 5  degree, 10% translation: {:.2f}".format(
            aps[
                -1,
                degree_thres_list.index(5),
                shift_thres_list.index(0.10),
                iou_thres_list.index(0.75),
            ]
            * 100
        )
    ...
    print(
        "IoU50, 5  degree, 20% translation: {:.2f}".format(
            aps[
                -1,
                degree_thres_list.index(5),
                shift_thres_list.index(0.20),
                iou_thres_list.index(0.50),
            ]
            * 100
        )
    )
    print(
        "IoU50, 10 degree, 10% translation: {:.2f}".format(
            aps[
                -1,
                degree_thres_list.index(10),
                shift_thres_list.index(0.10),
    ...
    print(
        "IoU50, 10 degree, 20% translation: {:.2f}".format(
            aps[
                -1,
                degree_thres_list.index(10),
                shift_thres_list.index(0.20),
    ...
    if logger is not None:
        ...
        logger.warning(
            "5 degree, 2cm: {:.1f}".format(
                pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(2)]
                * 100
            )
        )
        logger.warning(
            "5 degree, 5cm: {:.1f}".format(
                pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(5)]
                * 100
            )
        )
        ...
        logger.warning(
            "10 degree, 2cm: {:.1f}".format(
                pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(2)]
                * 100
            )
        )
        logger.warning(
            "10 degree, 5cm: {:.1f}".format(
                pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(5)]
                * 100
            )
        )
    else:

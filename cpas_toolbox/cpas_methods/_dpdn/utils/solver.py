...
class Solver(gorilla.solver.BaseSolver):
    def __init__(
        self, model, loss, dataloaders, logger, cfg, start_epoch=1, start_iter=0
    ):
        ...
        self.lr_scheduler = CyclicLR(
            ...
            base_lr=1e-5,
            max_lr=1e-3,
            step_size=cfg.max_epoch * cfg.num_mini_batch_per_epoch // 6,
            mode="triangular",
        )

        bnm_lmbd = lambda it: max(
            cfg.bn.bn_momentum * cfg.bn.bn_decay ** (int(it / cfg.bn.decay_step)),
            cfg.bn.bnm_clip,
        )
        self.bnm_scheduler = BNMomentumScheduler(
            self.model, bn_lambda=bnm_lmbd, last_epoch=self.iter
        )
        ...
        while self.epoch <= self.cfg.max_epoch:
            ...
            if self.epoch % 5 == 0:
                ...
                gorilla.solver.save_checkpoint(
                    ...
                    meta={"iter": self.iter, "epoch": self.epoch},
                )
    ...
    def train(self):
        ...
        for syn_data, real_data in zip(
            self.dataloaders["syn"], self.dataloaders["real"]
        ):
    ...
    def step(self, syn_data, real_data, mode):
        ...
        data = {
            ...
            "category_label": torch.cat(
                [syn_data["category_label"], real_data["category_label"]], dim=0
            ),
        }
    ...
    def update_scalar(self, list_name, list_value, index_counter, prefix):
        for name, value in zip(list_name, list_value):
            self.writer.add_scalar(
                prefix + name, float(value), self.list_couter[index_counter]
            )

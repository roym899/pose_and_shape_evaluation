...
def create_mesh_speedup(
    ...
    # 1/2 resolution
    samples_half = get_samples(
        int(N / 2), voxel_origin, voxel_size_half, transform=transform
    )
    sdf_values_half = infer_samples(
        decoder, latent_vec, samples_half, max_batch=max_batch
    )
    sdf_values_half_upsampled = upsample_cubic(sdf_values_half, int(N / 2), N)
    pos_samples, neg_samples, valid_samples = check_valid(
        sdf_values_half_upsampled, voxel_size_half
    )
    ...
    samples[valid_samples, 3] = infer_samples(
        decoder, latent_vec, samples[valid_samples, :], max_batch=max_batch
    )
    sdf_values = samples[:, 3]
    sdf_values = sdf_values.reshape(N, N, N)
...
def convert_sdf_samples_to_ply(
    ...
    if not silent:
        print(
            "Min: {0:.3f}, Max:{1:.3f}".format(
                numpy_3d_sdf_tensor.min(), numpy_3d_sdf_tensor.max()
            )
        )
    try:

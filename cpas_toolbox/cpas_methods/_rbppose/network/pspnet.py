...
class PSPNet(nn.Module):
    def __init__(self, bins=(1, 2, 3, 6), backend='resnet18', output_mask=True, output_channel=32):
        ...
        if output_mask:
            self.mask_final = nn.Sequential(nn.Conv2d(64, 2, kernel_size=1))
    ...
    def forward(self, x):
        ...
        p = self.up_3(p)
        if self.output_mask:
            mask = self.mask_final(p)
            # mask = self.log_softmax(mask)
        else:
            mask = None
        feature = self.final(p)
        return feature, mask
